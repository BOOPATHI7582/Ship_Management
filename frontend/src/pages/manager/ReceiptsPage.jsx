...
import { downloadManagerReceiptPdf, fetchAllReceipts, methodBadge, managerReceiptPdfUrl } from '../../api/receipts'
...
export default function ManagerReceiptsPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="font-display text-2xl font-bold text-navy-950">Receipts</h1>
        <p className="text-sm text-navy-500">
          Every verified payment generates a numbered receipt (REC-YYYY-NNNNNN) automatically.
        </p>
      </div>
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-navy-50 text-xs uppercase tracking-wide text-navy-500">
                <tr>
                  ...
                  <th className="px-5 py-3 hidden lg:table-cell">Invoice</th>
                  <th className="px-5 py-3 hidden sm:table-cell">Method</th>
                  <th className="px-5 py-3 hidden sm:table-cell">Issued</th>
              ...
              <tbody className="divide-y divide-navy-100">
                {data.content.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td className="hidden px-5 py-3.5 text-navy-600 md:table-cell">
                      {r.clientCompanyName || r.clientName || '—'}
                    </td>
                    <td className="hidden px-5 py-3.5 text-navy-600 lg:table-cell">{r.invoiceNo || '—'}</td>
                    <td className="hidden px-5 py-3.5 sm:table-cell">
                      <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${methodBadge(r.method)}`}>
                        {(r.method || '—').replace(/_/g, ' ')}
                      </span>
                    </td>
                    <td className="hidden px-5 py-3.5 text-navy-600 sm:table-cell">{formatDate(r.issuedOn)}</td>
                    <td className="px-5 py-3.5 text-right font-mono font-semibold text-emerald-700">
                      {money(r.amount, r.currency)}
                    </td>
                    <td className="px-5 py-3.5 text-right">
                      <button type="button" onClick={() => viewPdf(managerReceiptPdfUrl(r.id))} className={`${secondaryBtnCls} mr-2`}>
                        View
                      </button>
                      <button type="button" onClick={() => downloadManagerReceiptPdf(r.id, r.receiptNo)} className={secondaryBtnCls}>
                        Download
                      </button>
                    </td>
                  </tr>
                ...
                {data.content.length === 0 && (
                  <tr><td colSpan={7} className="px-5 py-8 text-center text-navy-400">No receipts issued yet.</td></tr>
                )}
              </tbody>
            </table>
          </div>
